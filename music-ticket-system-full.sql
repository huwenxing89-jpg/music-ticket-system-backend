...
-- 创建数据库
CREATE DATABASE IF NOT EXISTS `music_ticket_system` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `show` (
  ...
  `language` varchar(50) DEFAULT NULL COMMENT '语言（如：中文、英文等）',
  `director` varchar(100) DEFAULT NULL COMMENT '导演',
  `playwright` varchar(100) DEFAULT NULL COMMENT '编剧',
  `premiere_date` date DEFAULT NULL COMMENT '首演时间',
...
CREATE TABLE `session` (
  ...
  `show_time` datetime DEFAULT NULL COMMENT '演出时间',
  `start_time` datetime NOT NULL COMMENT '开始时间',
  `end_time` datetime NOT NULL COMMENT '结束时间',
  `duration` int DEFAULT NULL COMMENT '时长（分钟）',
  `price` decimal(10,2) NOT NULL COMMENT '价格',
  `vip_price` decimal(10,2) DEFAULT NULL COMMENT 'VIP票价',
  `normal_price` decimal(10,2) DEFAULT NULL COMMENT '普通票价',
  `student_price` decimal(10,2) DEFAULT NULL COMMENT '学生票价',
  `discount_price` decimal(10,2) DEFAULT NULL COMMENT '优惠票价',
  `price_types` varchar(100) DEFAULT NULL COMMENT '票价类型（vip,normal,student,discount）',
...
('admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iK6tZ5EH', '系统管理员', 'ADMIN', 1);

-- 插入测试用户账号（密码：123456）
INSERT INTO `user` (`username`, `password`, `nickname`, `role`, `status`) VALUES
('user', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iK6tZ5EH', '测试用户', 'USER', 1);
...
-- 创建数据库用户（如果不存在）
CREATE USER IF NOT EXISTS 'musicapp'@'localhost' IDENTIFIED BY 'Music@Ticket2024';
GRANT ALL PRIVILEGES ON music_ticket_system.* TO 'musicapp'@'localhost';
FLUSH PRIVILEGES;
...
SELECT COUNT(*) AS table_count FROM information_schema.tables WHERE table_schema = 'music_ticket_system';
SELECT '默认管理员账号: admin / 123456' AS admin_info;
SELECT '数据库用户: musicapp / Music@Ticket2024' AS db_user_info;
